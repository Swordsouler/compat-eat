...
export const FocusRestriction = () => {
    ...
    useEffect(() => {
        const food = sampleData.find(
            (food, index) => index === parseInt(id ?? "-1")
        );
        setFood(food);
        const filters = [];
        if (food !== undefined) {
            for (const filter of food.filters) {
                const filterObject = initialFilters.find(
                    (f) => f.name === filter
                );
        ...
        <>
            ...
            <div
                ...
                <img
                    src={food?.image}
                    alt={food?.name}
                    id='restriction__focus__image'
                />
                <p style={{ paddingLeft: "30px", paddingRight: "30px" }}>
                    This product is not compatible with some of your
                    restriction(s):
                </p>
                <div
                    id='filters'
