import { Divider, Fab, List, ListItem, ListItemButton, ListItemIcon, ListItemText, styled, Tab, Tabs, ToggleButton as MuiToggleButton } from '@mui/material';
...
export const Restrictions = () => {
    ...
    return (
        <>
            ...
            <Fab variant="extended" size="medium" color="primary" aria-label="add" id="add__restriction" onClick={() => navigate("/restrictions/add")}>
...
const RestrictionsList = ({foods, filters}: {foods: Food[], filters: Filter[]}) => {
    ...
    if(foods.length === 0) {
        return (
            <span style={{padding: "20px", display: "flex", justifyContent: "center"}}>No ingredients found with your restrictions !</span>
        );
    }
    return (
        <List style={{paddingBottom: "70px"}}>
            {foods.map((food, id) => {
                return (
                    <span key={food.name}>
                        <ListItem>
                            <ListItemButton onClick={() => navigate("/restrictions/focus/" + id)}>
                                <ListItemIcon>
